define(
    ...
    function (
        ...
        var instance = null;
        var countryData = storage.get('directory-data');
        var deliveryData = registry.get({ index: 'deliverydate_date' });
        ...
        return function (Shipping) {
            return Shipping.extend({
                ...
                getShippingMethodPrice: function () {
                    if (quote.shippingMethod()) {
                        return ' (' + priceUtils.formatPriceLocale(quote.shippingMethod().amount, quote.getBasePriceFormat()) + ')';
                    }
                    return ' (' + priceUtils.formatPriceLocale(0, quote.getBasePriceFormat()) + ')';
                },
                ...
                isShippingMethodAndDeliverDateSelected() {
                    if (deliveryData === undefined) { deliveryData = registry.get({ index: 'deliverydate_date' }); }
                    if (deliveryData.visible() === false) {
                        return true;
                    }
                    return deliveryData && deliveryData.visible() && deliveryData.value() !== '' && this.getShippingMethodPrice() !== '';
                },

                isShippingAddressSelected() {
                    let shippingAddrress = quote.shippingAddress();
                    return shippingAddrress.firstname
                        && shippingAddrress.lastname
                        && shippingAddrress.postcode
                        && shippingAddrress.street.length > 0
                        && shippingAddrress.city
                        && shippingAddrress.telephone;
                },

                getNameShippingMethod: function () {
                    if (deliveryData === undefined) { deliveryData = registry.get({ index: 'deliverydate_date' }); }
                    if (deliveryData && deliveryData.visible() && deliveryData.value() !== '') {
                        return viewUtils.getBlockTitle('shipping_method') + ' - ' + moment(deliveryData.value()).format('DD.MM.YY') + this.getShippingMethodPrice();
                    }
