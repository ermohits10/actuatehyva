 ...
 */
 ...
 define(
    [
        ...
        'Ced_Onbuy/js/modal/popup',
        'vkbeautify',
        'highlight'
    ], function ($, _, utils, registry, Column, confirm, loader, popup, vk, highlight) {
        'use strict';

        return Column.extend(
            {
                defaults: {
                    bodyTmpl: 'Ced_Onbuy/grid/cells/actions',
                    sortable: false,
                    draggable: false,
                    actions: [],
                    rows: [],
                        ...
                        $.ajax(
                            {
                                ...
                                success: function (response, status, request) {
                                    var size = request.getResponseHeader("Content-Length") / 1024;

                                    // Skipping files larger than 1MB
                                    if (size < 1024) {
                                        if (dataType === 'xml') {
                    ...
                    if (download === false) {
                        ...
                        if (data.render === 'html' && dataType === 'json') {
                            ...
                            if (result && Object.keys(result).length > 0) {
                                result = this.CreateTableView(result);
                            }
